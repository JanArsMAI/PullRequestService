...
0. ```git clone https://github.com/JanArsMAI/PullRequestService``` и ```cd .\PullRequestService\```
В целях простоты запуска из `.gitignore` были сняты ограничения на коммит конфигурационных файлов. 
